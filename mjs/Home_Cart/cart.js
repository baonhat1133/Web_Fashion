...
function updatePrdQuantity() {
    const idPrd = document.getElementsByClassName("btn-state");
...
var totalLocal = getlocalTotalPrd();
function changeQuantityLocal(index, state) {
...
//Delete product
function clickDelProduct() {
    const btn_del = document.getElementsByClassName("delItem");
...
function deleteLocal(idDel) {
    let productFilter;
    let totalDel = getlocalTotalPrd();
...
function totalPayment() {
    var totalPay = listLocal.reduce((total, product, i) => {
        return total += product.quantity * product.price;
    }, 0);
    renderInfoPayment(totalPay);
}
function renderInfoPayment(totalPay) {
    let htmlPay = "";
    let freeship = 0;
    if (totalPay >= 400000) {
        freeship = 40000;
